...
export class BonusServiceImpl implements BonusService {
  ...
  createBonus = () => {
    ...
    const amount =
      bonusesAmounts[Math.floor(Math.random() * bonusesAmounts.length)]
    const xPosition =
      Math.random() * (this._gameConfigurator.canvasWidth - CELL_SIZE)
    const yPosition =
      Math.floor(
        (Math.random() *
          (this._gameConfigurator.canvasHeight - TOOLBAR_HEIGHT)) /
          CELL_SIZE +
          1,
      ) *
        CELL_SIZE +
    ...
    this._bonuses.forEach((bonus, index) => {
      ...
      if (bonus && collision(this._gameConfigurator.mouse, bonus)) {
        this._gameConfigurator.balance =
          this._gameConfigurator.balance + bonus.amount
